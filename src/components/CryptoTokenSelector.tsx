...
export default function CryptoTokenSelector({
  ...
  // Prevent background scrolling when full-screen modal is open on mobile
  React.useEffect(() => {
    if (isOpen) {
    ...
    <div className="relative inline-block font-sans text-xs">
      ...
      {isOpen && (
        <>
          {/* Backdrop */}
          <div 
            className="fixed inset-0 z-40 bg-slate-950/75 backdrop-blur-sm sm:bg-slate-950/50" 
            onClick={() => setIsOpen(false)}
          />

          {/* Modal Container: Fullscreen on Mobile (<sm), Popover on Desktop (>=sm) */}
          <div className="fixed inset-0 z-50 w-full h-[100dvh] bg-slate-950 sm:bg-slate-900 border-0 sm:border sm:border-slate-700/80 rounded-none sm:rounded-2xl shadow-2xl overflow-hidden flex flex-col sm:absolute sm:inset-auto sm:left-0 sm:top-full sm:mt-2 sm:w-[580px] md:w-[700px] sm:h-auto sm:max-h-[85vh] animate-in fade-in zoom-in-95 duration-150">
            {/* Header with Search & Tabs */}
            <div className="p-3.5 sm:p-4 bg-slate-950/95 border-b border-slate-800 flex flex-col gap-2.5 shrink-0">
              <div className="flex items-center justify-between gap-2">
                <div className="flex items-center gap-2 min-w-0">
                  ...
                  <div className="min-w-0">
                    <h4 className="text-xs sm:text-sm font-black text-white truncate">
                      Pilih Aset Trading: Gold (XAUUSD) & Top Crypto 1 - 500
                    </h4>
                    <p className="text-[10px] text-slate-400 truncate">
                      Tersedia {CRYPTO_TOKENS.length} aset dengan multi-timeframe H4 + 5M Live Box
                    </p>
                  </div>
                </div>
                ...
                <button
                  onClick={() => setIsOpen(false)}
                  className="flex items-center gap-1 px-2.5 py-1.5 sm:p-1.5 bg-slate-800/80 hover:bg-slate-700 rounded-xl text-slate-300 hover:text-white transition cursor-pointer text-xs font-bold shrink-0 border border-slate-700"
                  aria-label="Tutup"
                >
                  <span className="sm:hidden text-[11px]">Tutup</span>
                  <X className="w-4 h-4" />
                </button>
              </div>
              ...
              <div className="relative mt-1">
                ...
                <input
                  type="text"
                  placeholder="Cari Emas, Ticker atau Nama (misal: XAU, Gold, BTC, SOL, SUI, PEPE)..."
                  value={searchQuery}
                  onChange={(e) => setSearchQuery(e.target.value)}
                  className="w-full bg-slate-900/90 border border-slate-700/80 rounded-xl pl-9 pr-8 py-2.5 sm:py-2 text-xs sm:text-sm text-white placeholder-slate-500 focus:outline-none focus:border-indigo-500 focus:ring-1 focus:ring-indigo-500"
                  autoFocus
                />
                {searchQuery && (
                  <button
              ...
              {/* Main Category Tabs - Scrollable on Mobile for Easy Swipe */}
            ...
            {/* Token List */}
            <div className="flex-1 overflow-y-auto p-3 sm:p-3 space-y-1.5 custom-scrollbar min-h-0">
              {filteredTokens.length === 0 ? (
                <div className="py-16 text-center text-slate-500">
                  <Search className="w-10 h-10 mx-auto mb-3 opacity-40 text-slate-400" />
                  <p className="text-xs font-semibold text-slate-300">Tidak ditemukan aset atau token yang sesuai pencarian.</p>
                  <p className="text-[11px] text-slate-500 mt-1">Coba cari dengan kata kunci lain atau pilih tab kategori di atas.</p>
                </div>
              ) : (
                  ...
                  {filteredTokens.map((token) => {
                    ...
                    return (
                      <button
                        ...
                        className={`w-full flex items-center justify-between p-3 sm:p-2.5 rounded-xl border text-left transition cursor-pointer active:scale-[0.98] ${
                          isSelected
                            ? isGold
                      ...
                      >
                        <div className="flex items-center gap-2.5 min-w-0">
                          <div className={`w-9 h-9 sm:w-8 sm:h-8 rounded-xl sm:rounded-lg flex items-center justify-center font-black text-xs shrink-0 ${
                            isGold
                          ...
                          <div className="min-w-0">
                            <div className="flex items-center gap-1.5">
                              <span className={`font-black text-xs sm:text-xs tracking-wide truncate ${
                            ...
                            <p className="text-[11px] sm:text-[10.5px] text-slate-400 truncate max-w-[170px] sm:max-w-[140px]">
                              {token.name}
                            </p>
                          </div>
                        </div>
                        ...
                        <div className="flex items-center gap-1.5 shrink-0 pl-2">
                          <span className={`text-[10px] sm:text-[9.5px] px-2 sm:px-1.5 py-0.5 rounded font-bold ${
                            isGold
            ...
            {/* Footer Summary - Responsive with mobile touch button */}
            <div className="p-3 sm:p-2.5 bg-slate-950 border-t border-slate-800 flex items-center justify-between text-xs sm:text-[11px] text-slate-400 px-4 shrink-0">
              <div className="flex items-center gap-1.5">
                <span className="w-2 h-2 rounded-full bg-emerald-400 animate-pulse"></span>
                <span><strong>{filteredTokens.length}</strong> / {CRYPTO_TOKENS.length} aset</span>
              </div>
              <button
                onClick={() => setIsOpen(false)}
                className="sm:hidden px-3 py-1 bg-indigo-600 hover:bg-indigo-500 text-white rounded-lg font-bold text-xs cursor-pointer shadow"
              >
                Selesai
              </button>
              <span className="hidden sm:inline-block text-slate-500">Live Dual 4H & 5M Streaming</span>
            </div>
          </div>
        </>
      )}
